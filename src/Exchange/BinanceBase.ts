...
export class BinanceBase {
    ...
    request = async (root: string, url: string, securityType: SecurityType, httpMethod: HttpMethod, data: ParsedUrlQueryInput = {}) => {
        let headers = {
        ...
        switch (securityType) {
            ...
            case SecurityType.USER_DATA:
                ...
                let signature = crypto.createHmac('sha256', this.API_SECRET).update(queryText).digest('hex');
